...
class DictionaryAddFragment : Fragment() {
    ...
    private lateinit var addDictionaryBtn: Button
    ...
    override fun onCreateView(
        ...
        val root = inflater.inflate(R.layout.fragment_dictionary_add, container, false)
        cl = root.findViewById(R.id.dictionary_add_parent)
        sv = root.findViewById(R.id.dictionary_add_scrollview)
        code = root.findViewById(R.id.add_dictionary_code)
        name = root.findViewById(R.id.add_dictionary_name)
        description = root.findViewById(R.id.add_dictionary_description)
        addDictionaryBtn = root.findViewById(R.id.add_dictionary_btn)
        addDictionaryBtn.setOnClickListener{
            addFaq()
        }
        val DictionariesListBtn: Button = root.findViewById(R.id.dictionary_list_btn)
        DictionariesListBtn.setOnClickListener{
    ...
    fun addFaq() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            val response = service.addDictionary(requestBody, "Bearer " + token)
            withContext(Dispatchers.Main) {
                if (response.isSuccessful) {
                    val jsonResponse = JsonParser.parseString(response.body()?.string()).toString()
                    val addData = Gson().fromJson<DictionaryResponse>(jsonResponse, DictionaryResponse::class.java)
